...
pattern = r"// ── Panel 1: Overview ──.*?_panel\(\s*child:\s*Column\(\s*crossAxisAlignment: CrossAxisAlignment\.start,\s*children: \[.*?Text\(\s*'Tổng quan thu nhập'.*?SizedBox\(height: 16\),.*?Row\(.*?Expanded\(.*?Container\(.*?// Balance.*?FittedBox\(.*?Text\(.*?\}\],\s*\),\s*\),\s*\],\s*\),"
