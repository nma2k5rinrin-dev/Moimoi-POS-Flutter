...
with open('lib/features/thu_chi/presentation/nhap_chi_page.dart', 'r', encoding='utf-8') as f:
    content = f.read()
...
old_cat_props = """  List<ThuChiCategory> _categories = [];
  ...
  final List<ThuChiCategory> _defaultCategories = [
    const ThuChiCategory(emoji: '🍽️', label: 'Nguyên liệu', color: AppColors.red500),
    ...
    const ThuChiCategory(emoji: '👥', label: 'Lương NV', color: Color(0xFF9333EA)),
    const ThuChiCategory(emoji: '📢', label: 'Marketing', color: AppColors.emerald500),
  ...
  Future<void> _loadCustomCategories() async {
    ...
    final String? jsonStr = prefs.getString('custom_chi_categories');
    ...
    if (jsonStr != null) {
      try {
        ...
        debugPrint('Error decoding custom chi categories: $e');
      }
    }
  ...
  Future<void> _saveCustomCategories() async {
    final prefs = await SharedPreferences.getInstance();
    // Lọc ra các mục isCustom = true (bỏ nút Thêm mới)
    final customCats = _categories.where((c) => c.isCustom).map((c) => c.toMap()).toList();
    await prefs.setString('custom_chi_categories', jsonEncode(customCats));
  }"""
  
new_cat_props = """  final List<ThuChiCategory> _defaultCategories = [
    const ThuChiCategory(type: 'chi', emoji: '🍽️', label: 'Nguyên liệu', color: AppColors.red500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '🔧', label: 'Biên mức', color: AppColors.blue500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '⏰', label: 'Tiền điện', color: AppColors.amber500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '🚚', label: 'Vận chuyển', color: AppColors.violet500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '🛠', label: 'Sửa chữa', color: AppColors.orange500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '👥', label: 'Lương NV', color: Color(0xFF9333EA), isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '📢', label: 'Marketing', color: AppColors.emerald500, isCustom: false),
    const ThuChiCategory(type: 'chi', emoji: '📦', label: 'Khác', color: AppColors.slate500, isCustom: false),
  ];

  List<ThuChiCategory> get _categories {
    final store = context.watch<AppStore>();
    final customCats = store.currentCustomThuChiCategories.where((c) => c.type == 'chi').toList();
    return [
      ..._defaultCategories,
      ...customCats,
      const ThuChiCategory(type: 'chi', emoji: '➕', label: 'Thêm mới', color: AppColors.slate400, isCustom: false),
    ];
  }"""
...
new_add_dialog = """  void _showAddCategoryDialog() {
    showAddCategoryDialog(
      ...
      onSave: (name, emoji, color) {
        ...
        store.addThuChiCategory(ThuChiCategory(type: 'chi', emoji: emoji, label: name, color: color, isCustom: true));
        setState(() {
...
old_update_dialog = """                    onSave: (name, emoji, color) {
                      setState(() {
                        _categories[_selectedCategory] = ThuChiCategory(
                            emoji: emoji, label: name, color: color, isCustom: true);
                      });
                      _saveCustomCategories();
                    },"""
    
# Actually let's just do a simpler replacement for update/delete, finding by part if needed.
# From the file previously viewed, wait I saw:
...
new_update_exact = """                    onSave: (name, emoji, color) {
                      ...
                      store.updateThuChiCategory(
                        ThuChiCategory(
                          id: cat.id, storeId: cat.storeId, type: 'chi',
                          emoji: emoji, label: name, color: color, isCustom: true
                        )
                      );
                      setState(() {
...
old_delete_exact = """                onTap: () {
                  ...
                  setState(() {
                    ...
                    // Rơi về category an toàn nếu đang chọn cái bị xóa
...
with open('lib/features/thu_chi/presentation/nhap_chi_page.dart', 'w', encoding='utf-8') as f:
    f.write(content)
