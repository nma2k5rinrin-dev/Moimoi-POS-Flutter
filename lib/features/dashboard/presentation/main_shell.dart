...
class _MainShellState extends State<MainShell> {
  ...
  static const List<_NavItem> _sadminMenuItems = [
    ...
    _NavItem(icon: Icons.notifications_none_rounded, label: 'Thông báo', path: '/'),
    _NavItem(icon: Icons.workspace_premium_rounded, label: 'Gói Premium', path: '/orders'),
...
Widget _buildAvatarWidget(AppStore store, double radius) {
  ...
  final letter = (store.currentUser?.username.isNotEmpty == true)
      ...
      : 'U';
  if (hasAvatar) {
    if (CloudflareService.isUrl(store.currentUser!.avatar)) {
      return ClipOval(
        child: CachedNetworkImage(
          imageUrl: store.currentUser!.avatar,
          width: radius * 2,
          height: radius * 2,
          fit: BoxFit.cover,
