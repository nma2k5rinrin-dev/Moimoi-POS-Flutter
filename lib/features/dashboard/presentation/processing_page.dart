...
class _ProcessingPageState extends State<ProcessingPage>
  ...
  Widget build(BuildContext context) {
    ...
    final allOrders = context.watch<OrderFilterStore>().visibleOrders;
    ...
    final completedCount = allOrders
        .where((o) => o.status == 'completed' && o.time.startsWith(todayStr))
        .length;
    final cancelledCount = allOrders
        .where((o) => o.status == 'cancelled' && o.time.startsWith(todayStr))
        .length;
...
class _OrderCardState extends State<_OrderCard> {
  ...
  Widget _buildExpandedBody(OrderModel order, OrderStore store) {
    return Column(
      children: [
        // Divider
                ...
                children: [
                  ...
                  if (order.status == 'pending')
                    Expanded(
                      child: GestureDetector(
                        onTap: () =>
                            store.updateOrderStatus(order.id, 'processing'),
                        child: Container(
                          padding: EdgeInsets.symmetric(vertical: 10),
                          decoration: BoxDecoration(
  ...
  void _showChangeTableDialog(
    ...
    final tables = context.watch<ManagementStore>().storeTables[context.watch<ManagementStore>().getStoreId()] ?? [];
  ...
  void _showPaymentQR(BuildContext context, OrderModel order, OrderStore store) {
                          ...
                          Row(
                            children: [
                              Expanded(
                                ...
                                HapticFeedback.lightImpact();
                                    Navigator.pop(ctx);
                                    store.completeOrderWithPayment(
                                      order.id,
                                      'cash',
                                    );
                                  },
  ...
  void _showAddItemsDialog(
    ...
    final allProducts = context.watch<InventoryStore>().currentProducts
        .where((p) => !p.isOutOfStock)
        .toList();
    final allCategories = context.watch<InventoryStore>().currentCategories;
