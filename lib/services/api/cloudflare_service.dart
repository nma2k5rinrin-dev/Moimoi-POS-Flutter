...
class CloudflareService {
  static const String _workerUrl = EnvConfig.cloudflareWorkerUrl;
  // _uploadSecret removed for security. We will pass Supabase JWT instead.
  ...
  static Future<String> uploadBytes({
    ...
    try {
      ...
      // Lấy Access Token từ phiên Supabase hiện tại thay vì Secret tĩnh
      final session = Supabase.instance.client.auth.currentSession;
      final accessToken = session?.accessToken ?? '';

      if (accessToken.isEmpty) {
        throw Exception('Người dùng chưa đăng nhập (Thiếu Session Token)');
      }
  ...
  static Future<String> uploadBase64({
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({'data': base64Data, 'folder': folder}),
      );
