...
class PrinterService extends ChangeNotifier {
  ...
  Future<bool> printReceipt(OrderModel order, StoreInfoModel storeInfo) async {
    ...
    try {
      ...
      // Header
      bytes += generator.text(storeInfo.name,
          styles: const PosStyles(align: PosAlign.center, bold: true, height: PosTextSize.size2, width: PosTextSize.size2));
      bytes += generator.text('');

      if (storeInfo.address.isNotEmpty) {
        bytes += generator.text(storeInfo.address,
            styles: const PosStyles(align: PosAlign.center));
      }
      if (storeInfo.phone.isNotEmpty) {
        bytes += generator.text('SĐT: ${storeInfo.phone}',
            styles: const PosStyles(align: PosAlign.center));
      }
      ...
      bytes += generator.row([
        ...
        PosColumn(text: 'Gia', width: 4, styles: const PosStyles(bold: true, align: PosAlign.right)),
      ]);
      bytes += generator.hr(ch: '-');
      ...
      for (final item in order.items) {
        bytes += generator.row([
          ...
          PosColumn(text: formatter.format(item.price * item.quantity), width: 4, styles: const PosStyles(align: PosAlign.right)),
        ]);
      ...
      // Total
      bytes += generator.row([
        PosColumn(text: 'TONG:', width: 6, styles: const PosStyles(bold: true, height: PosTextSize.size2)),
        PosColumn(text: '${formatter.format(order.totalAmount)}d', width: 6,
  ...
  Future<bool> printTest() async {
    ...
    try {
      ...
      bytes += generator.text('=== IN THU ===',
          styles: const PosStyles(align: PosAlign.center, bold: true, height: PosTextSize.size2, width: PosTextSize.size2));
      bytes += generator.text('');
      bytes += generator.text('Moimoi POS',
          styles: const PosStyles(align: PosAlign.center, bold: true));
      bytes += generator.text('May in hoat dong tot!',
          styles: const PosStyles(align: PosAlign.center));
      bytes += generator.text(
