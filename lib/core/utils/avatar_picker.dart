  ...
  if (ext.endsWith('.gif')) {
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Không hỗ trợ định dạng GIF. Vui lòng chọn ảnh JPG hoặc PNG.',
          ),
...
Future<String?> pickCropAndUploadAvatar(BuildContext context) async {
  ...
  try {
    ...
    debugPrint(
      '[pickCropAndUploadAvatar] Upload failed, falling back to base64: $e',
    );
  ...
  if (CloudflareService.isUrl(imageData)) {
    return ClipOval(
      child: CachedNetworkImage(
        ...
        fit: BoxFit.cover,
        placeholder: (_, _) =>
            _fallbackAvatar(radius, fallbackIcon, fallbackColor, fallbackBg),
        errorWidget: (_, _, _) =>
            _fallbackAvatar(radius, fallbackIcon, fallbackColor, fallbackBg),
      ),
    );
