...
import 'package:moimoi_pos/features/dashboard/logic/ui_store.dart';
import 'package:moimoi_pos/features/settings/logic/management_store.dart';

class AppStore extends ChangeNotifier with BaseMixin, AuthStore, InventoryStore, OrderStore, UIStore, ManagementStore {
  ...
  Future<void> loadInitialData(UserModel user) async {
    isLoading = true;
    notifyListeners();
    currentUser = user; // Ensure currentUser is set immediately
    try {
      ...
      var storeInfoQuery = _supabase.from('store_infos').select();
      if (storeId != null) {
        storeInfoQuery = storeInfoQuery.eq('store_id', storeId);
      }
      ...
      final results = await Future.wait([
        ...
        (storeId == null)
            ? Future.value([])
            : _supabase.from('store_tables').select().eq('store_id', storeId).order('sort_order'), // 2
        (storeId == null)
            ? Future.value([])
            : _supabase.from('categories').select().eq('store_id', storeId), // 3
        (storeId == null)
            ? Future.value([])
            : _supabase.from('products').select().eq('store_id', storeId), // 4
      ...
      orders = (results[5]).map((o) => OrderModel.fromMap(o)).toList();

      notifications =
          (results[6]).map((n) => NotificationModel.fromMap(n)).toList();

      upgradeRequests = (results[7])
      ...
      try {
        final ppData = await _supabase.from('premium_payments').select()
            .order('paid_at', ascending: false);
        premiumPayments = (ppData as List)
            .map((r) => PremiumPaymentModel.fromMap(r))
            .toList();
      } catch (e) {
        debugPrint('[loadData] premium_payments table may not exist yet: $e');
        premiumPayments = [];
      }
  ...
  Future<String> login(String username, String password) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('[login] Failed to update last_login_at: $e');
        // Continue anyway
      }
  ...
  Future<void> addStaff({
    ...
    try {
      ...
      if (role == 'admin') {
        final newStoreInfo = {
          'store_id': username,
          'name': storeName?.isNotEmpty == true ? storeName! : (fullname.isNotEmpty ? fullname : username),
          'phone': phone,
          'is_premium': false,
          'created_at': DateTime.now().toIso8601String(),
        };
        await _supabase.from('store_infos').insert(newStoreInfo);
  ...
  Future<void> approveUpgrade(String requestId) async {
    ...
    try {
      await _supabase.from('premium_payments').insert(paymentRecord);
      premiumPayments.insert(0, PremiumPaymentModel.fromMap(paymentRecord));
    } catch (e) {
      debugPrint('[approveUpgrade] Failed to record payment: $e');
    }
  ...
  void _setupRealtime(UserModel user, String? storeId) {
    ...
    _ordersChannel = _supabase.channel('orders-changes').onPostgresChanges(
      ...
      callback: (payload) async {
        if (storeId == null && user.role == 'sadmin') return; // Sadmin doesn't need global order updates
